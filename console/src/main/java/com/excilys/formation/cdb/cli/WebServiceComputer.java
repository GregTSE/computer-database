...
public class WebServiceComputer {
	...
	return   webTarget.request(MediaType.APPLICATION_JSON).get(new GenericType<List<ComputerDTO>>() {});
    }

    public ComputerDTO get(String id) {
	return webTarget.path(id).request(MediaType.APPLICATION_JSON).get(new GenericType<ComputerDTO>() {});
    }
    
    public void add(ComputerDTO computerDTO) {	
	webTarget.path("/add").request().post(Entity.entity(computerDTO, "application/json"));
    }
}